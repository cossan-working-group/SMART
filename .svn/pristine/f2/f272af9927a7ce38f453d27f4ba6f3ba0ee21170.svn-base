function [VrobustOutput, VlowerBound, VupperBound, CXann, MOutput]=applyBMS(varargin)
...
AdjustmentFactor_var= sum(repmat(VposteriorP,size(MOutput,1),1).*(MOutput-repmat(VrobustOutput,[1,Nnets])).^2,2);
