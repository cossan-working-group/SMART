function [VrobustOutput, VlowerBound, VupperBound, CXann,CPostDist]=applyAdaptiveBMS(varargin)
...
for k=1:2:length(varargin)
    switch lower(varargin{k})
        ...
        case {'ccalibrationdata','ccalibration'} % preferred size Nsamples x Ninput (assuming Nsamples>Ninput)
            CcalibrationData  = varargin{k+1}; 
        case {'vmapping'} % preferred size Nsamples x Ninput (assuming Nsamples>Ninput)
...
% Merge the initial datasets
 %do not change the order! ;MtestData
%MdataIN(MdataIN(:,38)==0,:)=[];
if Lset2build
    CXann=cell(1,Nnets);
else
...
for inet=1:Nnets
    ...
    elseif ~isempty(mapping)
        MdataIN=CcalibrationData{mapping(inet)};
    end
    ...
    if Lpost2compute
        ...
        VbreakSize      = unique(MdataIN(:,38));
        Vvariance       = zeros(length(VbreakSize),Nnets);
